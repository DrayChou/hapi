...
// Skip this test on Windows due to file watcher behavior differences
const isWindows = process.platform === 'win32'

const testSuite = isWindows ? describe.skip : describe

testSuite('sessionScanner', () => {
  let testDir: string
  ...
  beforeEach(async () => {
    ...
    // Get only the directory name, not the full path, for cross-platform compatibility
    // On Windows: C:\Users\...\Temp\scanner-test-xxx -> scanner-test-xxx
    // On Unix: /tmp/scanner-test-xxx -> scanner-test-xxx
    const testDirName = require('node:path').basename(testDir)
    const projectName = testDirName.replace(/[\/\\]/g, '-')
